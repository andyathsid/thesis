...
\begin{enumerate}[nosep]
    ...
    \item p95 (Persentil ke-95): Merepresentasikan ambang batas waktu di mana 95\% dari total sampel inferensi lebih cepat dari nilai tersebut. Metrik ini digunakan untuk mengukur performa dalam skenario terburuk (\textit{tail latency}), memastikan sistem tetap responsif bahkan saat memproses citra yang kompleks atau dalam kondisi beban tinggi.
    \begin{equation}
...
Alat deteksi \textit{open-vocabulary} menggunakan model OWLv2 yang dikembangkan oleh Google \parencite{mindererScalingOpenVocabularyObject2024}. Sistem menggunakan varian model \texttt{google/\allowbreak owlv2-base-\allowbreak patch16} yang telah dikonversi ke format ONNX. Model ini mengintegrasikan \textit{vision transformer} (ViT) dengan \textit{text encoder} untuk melakukan deteksi objek berdasarkan deskripsi tekstual secara \textit{zero-shot}. Gambar \ref{fig:arsitektur_owlv2} menyajikan visualisasi arsitektur dari model OWLv2.
...
\begin{figure}[H]
    ...
    \caption{Distribusi Kelas pada Dataset PlantWild dari \protect\textcite{weiBenchmarkingInthewildMultimodal2024}}
...
\begin{table}[H]
    ...
    \multirow{2}{*}{\textbf{Vektor}} & \texttt{text} & 512 & Representasi semantik deskripsi gejala (RoBERTa). \\ \cline{2-4} 
     & \texttt{image} & 512 & Representasi fitur visual citra tanaman (Swin-T/SCOLD). \\ \hline
    \multirow{6}{*}{\textbf{Payload}} & \texttt{plant\_name} & String & Nama spesies tanaman. Diindeks secara \textit{full-text} untuk filtrasi. \\ \cline{2-4} 
     & \texttt{label} & String & Nama spesifik penyakit. Diindeks secara \textit{full-text}. \\ \cline{2-4} 
     & \texttt{caption} & String & Deskripsi tekstual lengkap mengenai gejala. \\ \cline{2-4} 
     & \texttt{image\_url} & String & Tautan publik citra di Cloudflare R2. \\ \cline{2-4} 
     & \texttt{source\_id} & Integer & ID referensi internal. \\ \cline{2-4} 
...
\begin{table}[H]
    ...
    \multirow{2}{*}{\textbf{Vektor Padat}} & \texttt{gemini} & 3.072 & Representasi semantik dari Gemini Embedding. \\ \cline{2-4} 
     & \texttt{openai} & 3.072 & Representasi semantik dari OpenAI Embedding. \\ \hline
    \multirow{2}{*}{\textbf{Vektor Jarang}} & \texttt{splade} & Variabel & Representasi leksikal dari SPLADE++. \\ \cline{2-4} 
     & \texttt{bm25} & Variabel & Skor relevansi berbasis frekuensi kata (BM25). \\ \hline
    \multirow{5}{*}{\textbf{Payload}} & \texttt{page\_content} & String & Konten teks lengkap dari dokumen. \\ \cline{2-4} 
     & \texttt{metadata.plant} & String & Nama tanaman. Diindeks secara \textit{full-text}. \\ \cline{2-4} 
     & \texttt{metadata.disease} & String & Nama penyakit. Diindeks secara \textit{full-text}. \\ \cline{2-4} 
     & \texttt{metadata.type} & String & Jenis dokumen (budidaya/penyakit). \\ \cline{2-4} 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{bab3/arsitekturantarmukasistem.png}
...
Pengujian antarmuka dilakukan menggunakan metode \textit{Black Box Testing} untuk memastikan bahwa seluruh fungsionalitas yang telah dirancang pada \textit{Use Case Diagram} dapat beroperasi sesuai dengan ekspektasi pengguna. textit{Black Box Testing} merupakan metode pengujian perangkat lunak yang berfokus pada spesifikasi fungsional sistem \parencite{bhasinBlackBoxTesting2014}. Pengujian fungsionalitas antarmuka dirangkum dalam Tabel \ref{tab:black_box_testing_expected}.
...
\begin{table}[H]
    ...
    \textbf{UC1} & Mengirim Prompt & Pengguna mengetik instruksi pada kolom teks (\textit{textarea}) dan menekan tombol kirim (ikon \texttt{ArrowUp}). & Sistem mengirim pesan ke server dan menampilkan respons agen secara bertahap (\textit{streaming}). \\ \hline
    \textbf{UC2} & Mengunggah Citra Tanaman & Pengguna mengklik ikon \texttt{Paperclip} untuk memilih gambar atau menyeret berkas langsung ke area percakapan. & Gambar muncul sebagai \textit{preview} di atas kolom input dan berhasil dikirim bersama pesan teks. \\ \hline
    ...
    \textbf{UC5} & Melihat Transparansi Agen & Pengguna menekan tombol kontrol alat (ikon \texttt{Wrench}) untuk mengaktifkan visibilitas proses. & Detail pemanggilan alat (\textit{tool calls}) dan hasil observasi internal agen ditampilkan di antara pesan chat. \\ \hline
    \end{tabular}
    }
\end{table}

\section{Pengujian dan Evaluasi Sistem Agentik}
...
Evaluasi dalam penelitian ini dilakukan dengan metode evaluasi \textit{offline}. Dalam konteks ini, istilah \textit{offline} merujuk pada prosedur evaluasi yang dijalankan menggunakan dataset terkurasi selama fase pengembangan untuk mengukur tolok ukur performa (\textit{benchmark}) dan mendeteksi regresi atau penurunan kinerja sistem, bukan evaluasi secara waktu nyata (\textit{real-time}) pada interaksi pengguna asli di lingkungan produksi. Penilaian kualitas sistem dilakukan menggunakan pendekatan \textit{LLM-as-a-judge}, yang memanfaatkan kemampuan model bahasa besar sebagai evaluator otomatis. Model Gemini 3 Pro dipilih sebagai evaluator utama karena kemampuan penalarannya yang unggul, sementara Gemini 2.5 Pro digunakan sebagai cadangan apabila terjadi kendala batasan laju permintaan (\textit{rate limit}). Selain LangSmith, proses ini didukung oleh pustaka evaluasi lain, yakni RAGAS dan DeepEval.
...
\paragraph{\textit{Dataset In-Distribution} (ID)}
...
\paragraph{Dataset Out-of-Distribution (OOD)}
...
\begin{table}[H]
    ...
    7. & Gandum & 3 & \textcite{getachewWheatLeafDataset2021c} \\ \hline
...
Hasil akhir dari proses generasi ini disimpan dalam format JSON yang terstruktur. Struktur ini mencakup input simulasi pengguna, luaran referensi yang diharapkan (\textit{ground truth}), serta metadata pendukung. Contoh entri dataset hasil generasi untuk ketiga skenario disajikan pada Gambar \ref{fig:synthetic_dataset_examples}.
...
Evaluasi pada pengujian satu tahap dilakukan menggunakan serangkaian metrik yang dirancang secara spesifik untuk mengukur kinerja sistem agentik pada berbagai aspek. Mayoritas metrik yang digunakan dalam evaluasi ini, yaitu \textit{Disease Accuracy}, \textit{Trajectory Accuracy}, \textit{Agent Goal Accuracy}, dan \textit{Faithfulness}, merupakan metrik berbasis \textit{LLM-as-a-judge} yang bersifat \textit{self-explaining}. Hal ini berarti bahwa selain menghasilkan skor kuantitatif, metrik-metrik tersebut juga menyertakan penjelasan kualitatif yang mendasari penilaian yang diberikan, sehingga memberikan transparansi lebih dalam proses evaluasi. Lebih lanjut, meskipun setiap metrik menerima masukkan dataset yang berbeda-beda, seluruh evaluator dirancang untuk menerima masukan citra dari pengguna. Hal ini memungkinkan evaluator untuk melakukan validasi sistem secara multimodal dengan mempertimbangkan bukti visual dalam penilaiannya. Penjelasan rinci mengenai metrik-metrik tersebut dijabarkan sebagai berikut.
...
\begin{table}[H]
    ...
    \multirow{2}{*}{\textbf{Input}} & \texttt{user\_text} & Teks pertanyaan atau keluhan yang diberikan pengguna. \\ \cline{2-3} 
     & \texttt{image\_url} & Tautan citra tanaman yang diunggah pengguna. \\ \hline
    \textbf{Output} & \texttt{final\_answer} & Jawaban akhir yang dihasilkan oleh agen. \\ \hline
    \multirow{3}{*}{\textbf{Referensi}} & \texttt{expected\_disease} & Nama penyakit yang sebenarnya (\textit{ground truth}) atau label "TANAMAN SEHAT". \\ \cline{2-3} 
     & \texttt{pathogen\_type} & Jenis patogen penyebab penyakit (misal: Fungi, Bakteri, Virus). \\ \cline{2-3} 
     & \texttt{reference\_answer} & Jawaban referensi yang lengkap dan benar sebagai pembanding. \\ \hline
    \end{tabular}
\end{table}

Alur logika penilaian dalam DAG dirancang secara hierarkis. Evaluasi dimulai dengan pengecekan apakah kasus tersebut merupakan tanaman sehat yang berhasil diidentifikasi dengan benar. Jika tidak, evaluasi berlanjut ke tingkat ketepatan diagnosis penyakit, mulai dari ketepatan penyakit utama, kecocokan parsial (diagnosis diferensial), hingga ketepatan identifikasi jenis patogen. Terakhir, jika sistem gagal melakukan identifikasi, metrik menilai apakah sistem melakukan penundaan jawaban (\textit{abstention}) secara tepat karena ketidakcukupan bukti. Struktur alur logika ini divisualisasikan pada Gambar \ref{fig:disease_accuracy_dag}.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        font=\small,
        start/.style={circle, draw, fill=blue!10, minimum size=1cm, align=center},
        decision/.style={diamond, draw, fill=yellow!10, aspect=2, inner sep=0pt, minimum width=3cm, align=center},
        verdict/.style={rectangle, draw, rounded corners, fill=green!10, minimum width=2.5cm, minimum height=0.8cm, align=center},
        fail/.style={rectangle, draw, rounded corners, fill=red!10, minimum width=2.5cm, minimum height=0.8cm, align=center},
        arrow/.style={-stealth, thick}
    ]

    \node (start) [start] {Mulai};
    \node (healthy) [decision, below=of start] {Identifikasi\\Sehat Benar?};
    
    \node (score1a) [verdict, right=1.5cm of healthy] {Skor 1.0\\(Benar Sehat)};
    
    \node (primary) [decision, below=of healthy] {Penyakit Utama\\Benar?};
    \node (score1b) [verdict, right=1.5cm of primary] {Skor 1.0\\(Benar Penyakit)};
    
    \node (partial) [decision, below=of primary] {Parsial/Kategori\\Benar?};
    \node (score075) [verdict, right=1.5cm of partial] {Skor 0.75\\(Parsial)};
    
    \node (pathogen) [decision, below=of partial] {Tipe Patogen\\Benar?};
    \node (score05) [verdict, right=1.5cm of pathogen] {Skor 0.5\\(Patogen Benar)};
    
    \node (abstain) [decision, below=of pathogen] {Abstain\\Tepat?};
    \node (score025) [verdict, right=1.5cm of abstain] {Skor 0.25\\(Abstain Tepat)};
    
    \node (score0) [fail, below=of abstain] {Skor 0.0\\(Salah Total)};

    \draw [arrow] (start) -- (healthy);
    
    \draw [arrow] (healthy) -- node[above] {Ya} (score1a);
    \draw [arrow] (healthy) -- node[right] {Tidak} (primary);
    
    \draw [arrow] (primary) -- node[above] {Ya} (score1b);
    \draw [arrow] (primary) -- node[right] {Tidak} (partial);
    
    \draw [arrow] (partial) -- node[above] {Ya} (score075);
    \draw [arrow] (partial) -- node[right] {Tidak} (pathogen);
    
    \draw [arrow] (pathogen) -- node[above] {Ya} (score05);
    \draw [arrow] (pathogen) -- node[right] {Tidak} (abstain);
    
    \draw [arrow] (abstain) -- node[above] {Ya} (score025);
    \draw [arrow] (abstain) -- node[right] {Tidak} (score0);
...
\begin{table}[H]
    ...
    \hline
    \textbf{Kategori} & \textbf{Parameter} & \textbf{Deskripsi} \\ \hline
    \multirow{2}{*}{\textbf{Input}} & \texttt{user\_text} & Instruksi atau pertanyaan teks dari pengguna. \\ \cline{2-3} 
     & \texttt{image\_url} & Tautan citra tanaman yang menjadi konteks visual analisis. \\ \hline
    \textbf{Aktual} & \texttt{trace\_messages} & Riwayat lengkap langkah eksekusi (\textit{trace}) dan pemanggilan alat oleh agen. \\ \hline
    \textbf{Referensi} & \texttt{reference\_tool\_calls} & Daftar alat yang diharapkan dipanggil untuk menyelesaikan tugas secara optimal (Standar Emas). \\ \hline
    \end{tabular}
    }
\end{table}
...
\begin{table}[H]
    ...
    & Substitusi & \texttt{knowledgebase\_search} dan \texttt{web\_search} dianggap setara sebagai alat pencarian informasi. \\ \hline
...
\begin{table}[H]
    ...
    \textbf{Kategori} & \textbf{Parameter} & \textbf{Deskripsi} \\ \hline
    \multirow{2}{*}{\textbf{Input}} & \texttt{user\_text} & Instruksi atau pertanyaan teks dari pengguna. \\ \cline{2-3} 
     & \texttt{image\_url} & Tautan citra masukan (multimodal). \\ \hline
    \multirow{2}{*}{\textbf{Aktual}} & \texttt{final\_answer} & Jawaban akhir yang dihasilkan oleh agen. \\ \cline{2-3}
     & \texttt{tools\_called} & Daftar alat yang dipanggil agen beserta keluarannya. \\ \hline
    \textbf{Referensi} & \texttt{reference\_goal} & Tujuan referensi yang diharapkan (misal: identifikasi, panduan, atau penolakan). \\ \hline
    \end{tabular}
    }
\end{table}
...
\begin{table}[H]
    ...
    \textbf{Kriteria} & Evaluasi keberhasilan agen mencapai \texttt{reference\_goal} berdasarkan output aktual dan penggunaan alat. Penilaian mencakup aspek identifikasi penyakit, panduan manajemen, analisis gejala, ekspresi ketidakpastian, dan penolakan permintaan di luar jangkauan. \\ \hline
...
\begin{table}[H]
    ...
    \hline
    \textbf{Kategori} & \textbf{Parameter} & \textbf{Deskripsi} \\ \hline
    \multirow{2}{*}{\textbf{Input}} & \texttt{user\_text} & Pertanyaan atau instruksi teks dari pengguna. \\ \cline{2-3} 
     & \texttt{image\_url} & Tautan citra tanaman sebagai dasar validasi observasi visual agen. \\ \hline
    \multirow{2}{*}{\textbf{Aktual}} & \texttt{final\_answer} & Jawaban akhir yang dihasilkan oleh agen. \\ \cline{2-3}
     & \texttt{retrieval\_context} & Kumpulan potongan informasi teks yang diekstraksi dari keluaran seluruh alat (pencarian, deteksi, dan klasifikasi). \\ \hline
    \end{tabular}
    }
\end{table}
...
\begin{table}[H]
    ...
    3. \textbf{Klaim Spesifik}: Identifikasi penyakit, nama patogen, dan pengobatan kimiawi HARUS didasarkan pada \texttt{retrieval\_context}. \newline
...
\begin{table}[H]
    ...
    \hline
    \textbf{Kategori} & \textbf{Parameter} & \textbf{Deskripsi} \\ \hline
    \multirow{2}{*}{\textbf{Input}} & \texttt{user\_text} & Teks pertanyaan atau instruksi dari pengguna. \\ \cline{2-3} 
     & \texttt{image\_url} & Tautan citra tanaman. Jika tersedia, indikator visual ditambahkan ke \textit{prompt} evaluator untuk konteks. \\ \hline
    \textbf{Aktual} & \texttt{retrieved\_contexts} & Daftar potongan teks yang diekstraksi dari riwayat pemanggilan alat (\textit{trace}), mencakup hasil pencarian dokumen maupun deteksi. \\ \hline
    \end{tabular}
    }
\end{table}
